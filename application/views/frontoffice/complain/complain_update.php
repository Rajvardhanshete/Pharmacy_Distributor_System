...
<div class="main-content-wrap sidenav-open d-flex flex-column">
    ...
    <div class="main-content">
        ...
        <hr>
        <form role="form" id="Form" action="<?php echo base_url('update/'.(isset($data -> id)?'updateComplain':'insertComplain'))?>" method="post">
                                    <div class="row">
                                        <div  class="col-md-3 form-group mb-3">
                                        <input name="id" id="id" type="hidden"   value="<?= isset($data->id)? $data->id: ''?> " />
                                            <label for="ComplainName">Complain Name</label>
                                            <input class="form-control" id="ComplainName" type="text"  name="ComplainName" value="<?= isset($data->ComplainName)? $data->ComplainName:''?> "/>
                                        </div>

                                        <div class="col-md-2 form-group mb-3">
                                        <label for="Mobile">Mobile</label>
                                        <input class="form-control" id="Mobile" type="text" name="Mobile" value="<?= isset($data->Mobile)? $data->Mobile:''?> "/>
                                        </div>

                                        <div class="col-md-2 form-group mb-3">
                                            <label for="Date">Date</label>
                                            <input class="form-control" type="date" id="Date" name="Date" value="<?= isset($data->Date)? $data->Date:date('Y-m-d');?>"/>
                                        </div>
...
// Check if no gender is selected
        ...
        else 
        {
            if(id > 0)
            {
                ...
                $.ajax({
                    url: base_path + "FrontOffice/complain/updateComplain",
                    type: "POST",
                    data: formData,
                    processData: false,
             contentType: false,
